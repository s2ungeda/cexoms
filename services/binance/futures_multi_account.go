...
type FuturesWebSocketManager struct {
	orderBookStreams map[string]*futures.WsDepthHandler
	tradeStreams     map[string]*futures.WsAggTradeHandler
	userDataStream   *futures.WsUserDataHandler
	markPriceStream  *futures.WsMarkPriceHandler
	positionStream   *futures.WsPositionHandler
}

// NewBinanceFuturesMultiAccount creates a new multi-account Binance Futures connector
func NewBinanceFuturesMultiAccount(accountManager types.AccountManager, testnet bool) *BinanceFuturesMultiAccount {
...
func (b *BinanceFuturesMultiAccount) connectAccount(ctx context.Context, account *types.Account) error {
	...
	b.wsManagers[account.ID] = &FuturesWebSocketManager{
		orderBookStreams: make(map[string]*futures.WsDepthHandler),
		tradeStreams:     make(map[string]*futures.WsAggTradeHandler),
	}
...
func (b *BinanceFuturesMultiAccount) Disconnect() error {
	...
	for accountID, wsManager := range b.wsManagers {
		...
		if wsManager.markPriceStream != nil {
			wsManager.markPriceStream.Close()
		}
		
		// Close position stream
		if wsManager.positionStream != nil {
			wsManager.positionStream.Close()
		}
...
func (b *BinanceFuturesMultiAccount) convertPosition(pos *futures.AccountPosition, accountID string) *types.Position {
	...
	unRealizedProfit, _ := decimal.NewFromString(pos.UnRealizedProfit)
...
func (b *BinanceFuturesMultiAccount) SubscribeOrderBook(symbol string, callback types.OrderBookCallback) error {
	...
	// Store handler
	wsManager.orderBookStreams[symbol] = &futures.WsDepthHandler{
		Done: done,
		Stop: stop,
	}
...
func (b *BinanceFuturesMultiAccount) SubscribeUserData() error {
	...
	for accountID, client := range b.clients {
		...
		wsManager.userDataStream = &futures.WsUserDataHandler{
			Done: done,
			Stop: stop,
		}
...
func (b *BinanceFuturesMultiAccount) getPositionSide(quantity decimal.Decimal) types.PositionSide {
